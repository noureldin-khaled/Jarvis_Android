package com.iot.guc.jarvis;

public class Constants {
    public static final int NO_INTERNET_CONNECTION = 0;
    public static final int APP_FAILURE = -1;
    public static final int SERVER_NOT_REACHED = -500;
    public static final int PERMISSION_CODE = 1;
    public static final String PACKAGE_NAME = "com.iot.guc.jarvis";
    public static final int NO_ENCRYPTION = 0;
    public static final int RSA_ENCRYPTION = 1;
    public static final int AES_ENCRYPTION = 2;
    public static final int NO_HEADERS = 0;
    public static final int AUTH_HEADERS = 1;
    public static final int USERNAME_HEADERS = 2;

}
